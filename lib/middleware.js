      ...
      fs.readFile(bundledReloadClientFile, function (err, src) {
        if (err) {
          if (shouldUseBundledLiveReload) {
            console.error('WARN: Bundled LiveReload client could not be found, ' +
                'falling back to bundle the client on demand. This may be because ' +
                'you are using a git cloned version of budo instead of one installed via ' +
                'npm.')
          }
