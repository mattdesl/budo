  ...
  middlewares.forEach(function (middleware) {
    if (typeof middleware !== 'function') {
      if (typeof middleware === 'string') {
        try {
          // Try and require module by name
          middleware = require(middleware)
        } catch (err) {
          try {
            // Fallback to resolving path relative to cwd
            middleware = require(path.resolve(opts.cwd, middleware))
          } catch (err) {
            throw new Error('the middleware "' + middleware + '" could not be found')
          }
        }
      } else {
