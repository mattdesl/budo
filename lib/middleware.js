...
function budoMiddleware (entryMiddleware, opts) {
  ...
  var cors = opts.cors
  var handler = stacked()
  var middlewares = parseMiddlewareOptions(opts.middleware)
  ...
  function parseMiddlewareOptions (list) {
    return [].concat(list)
      .filter(Boolean)
      .map(function (middleware) {
        var fnOpts = { cwd: cwd, dir: opts.dir }
        if (Array.isArray(middleware)) {
          // support browserify [ foo, { opt } ] style parsing
          fnOpts = middleware[1]
          middleware = middleware[0]
        } else if (typeof middleware === 'object' && '_' in middleware) {
          // support CLI subarg -m [ foo --opt ]
          var name = middleware['_'][0]
          fnOpts = xtend(fnOpts, middleware)
          delete fnOpts['_'] // remove main key
          middleware = name
        }
        ...
        if (typeof middleware === 'string') {
          ...
          var middlewareFunc
          try {
            middlewareFunc = require(middlewarePath)
          } catch (err) {
          ...
          middleware = middlewareFunc(fnOpts)
          if (typeof middlewareFunc !== 'function') {
            console.error(color.red('ERROR'), 'middleware "' + moduleName + '" function does not return a function')
            return null
          }
