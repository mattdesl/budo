...
function budoMiddleware (entryMiddleware, opts) {
  ...
  function liveReloadHandler (req, res, next) {
    ...
    var scriptUrl = live.path || '/budo/livereload.js'
    if (req.url === scriptUrl) {
      res.statusCode = 200
      res.setHeader('Content-Type', 'application/javascript')
      var liveScripts = [].concat(live.include).filter(Boolean)
      var cache = live.cache !== false
      var debug = live.debug

      // Default setup - use a bundled JS file for LiveReload client
      if (cache && !debug && liveScripts.length === 0) {
        console.log('using bundled')
        if (liveReloadClient) {
          console.log('using cached bundled')
          res.end(liveReloadClient)
        } else {
          console.log('using fresh bundled')
          fs.readFile(bundledReloadClientFile, function (err, src) {
            if (err) {
              console.error('Error bundling LiveReload client:\n' + err.message)
              res.statusCode = 500
              res.end('Error reading bundled LiveReload script: ' + err)
            } else {
      ...
      if (cache && liveReloadClient) {
        ...
        console.log('using fresh')
        var b = browserify({ debug: debug })
        var filePath = debug ? liveReloadClientFile : bundledReloadClientFile
        ...
        b.bundle(function (err, src) {
          if (err) {
            console.error('Error bundling LiveReload client:\n' + err.message)
            res.statusCode = 500
            res.end('Error with LiveReload script: ' + err)
          } else {
            console.log('finished bundling')
            liveReloadClient = src
            res.end(src)
