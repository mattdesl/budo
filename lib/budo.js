...
module.exports = function() {
  ...
  function runWatchify(entries, output, opt) {    
    ...
    watchify = createWatchify(watchifyArgs, { 
      ...
      delay: typeof opt.delay === 'number' ? opt.delay : 0,
      outfile: output.from
    })
      
    watchify.on('bundle:pending', function() {
      console.log("pending")
    })
    watchify.on('bundle:start', function() {
      console.log("start")
    })
    watchify.on('bundle:end', function() {
      console.log("end")
    })
  }
  ...
  function assignOptions(output, opt) {
    ...
    //TODO: get rid of this since it introduces some confusion
    // (esp. related to 'serving dir' and 'output dir' which may be different)
