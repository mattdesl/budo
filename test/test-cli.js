...
test('should run on available port', function(t) {
  t.plan(1)
  var proc = spawn(cliPath, ['app.js', '-o', 'bundle.js'], {
    cwd: __dirname,
    env: process.env
  })
...
test('entry mapping to bundle2.js', function(t) {
  runBundleMatch(t, {
    ...
    budo: ['app.js:bundle.js', '--no-debug'],
    to: 'bundle.js'
  })
})
...
function runBundleMatch(t, opt) {
  ...
  //the expected bundle
  var watchifyProc = npmSpawn('watchify ' + opt.watchify.join(' '), {
    cwd: cwd,
    env: process.env
  })
  ...
  function watchifyDone(msg) {
    ...
    fs.readFile(bundleExpected, 'utf8', function(err, data) {
      if (err)
        t.fail(err)
      budoMatches(data)
    })
  ...
  function budoMatches(source) {
    var proc = spawn(cliPath, opt.budo, {
      cwd: cwd,
      env: process.env
    })
      ...
      .on('data', function(data) {
        ...
        if (idx >= 0) {
          ...
          setTimeout(function() { //let bundling finish
            ...
            }, function(err, resp, data2) {
              if (err) t.fail(err)
              t.equal(data2, source, 'bundle matches')
              kill(proc.pid)
            })
          }, 1000)
        } else if (!foundMsg) {
